...
def help_screen():
    result = subprocess.run([sys.executable, str(PROJECT_ROOT / "steam_monitor.py"), "--help"], cwd=PROJECT_ROOT, capture_output=True, text=True, check=False)
...
def test_the_argument_groups_use_the_shared_names_in_order(help_screen):
    titles = ['Configuration & dotenv files', 'API settings', 'Email notifications', 'Webhook notifications', 'Intervals & timers', 'User information & listing', 'Features & output']

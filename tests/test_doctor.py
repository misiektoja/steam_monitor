...
def test_a_rate_limiting_interval_is_warned_about(monkeypatch, doctor_globals):
    monkeypatch.setattr(monitor, "STEAM_ACTIVE_CHECK_INTERVAL", 5)
    ...
    assert str(monitor.DOCTOR_MIN_SAFE_ACTIVE_INTERVAL) in rows[0].advice.fix


# The default intervals are safe, so the row must stay away rather than warning about every run
def test_a_safe_interval_is_not_warned_about(monkeypatch, doctor_globals):
    monkeypatch.setattr(monitor, "STEAM_ACTIVE_CHECK_INTERVAL", monitor.DOCTOR_MIN_SAFE_ACTIVE_INTERVAL)

...
# Stops credential validation on a local resource failure before offering more network work
def test_credential_resource_failure_stops(monkeypatch):
    import errno
    requests_seen = []

    # Raises a local descriptor error at the real Requests transport boundary
    def respond(adapter, request, **kwargs):
        requests_seen.append(request)
    ...
    with pytest.raises(SystemExit) as error:
        monitor.validate_steam_api_key("0123456789abcdef0123456789abcdef")

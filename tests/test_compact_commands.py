...
def test_display_prefix_is_short(monkeypatch, host, executable, method):
    ...
    monkeypatch.setattr(sys, "argv", ["/private/long/tools/steam_monitor.py" if method == "manual" else "/private/long/bin/steam_monitor"])
    monkeypatch.setattr(monitor, "__file__", "/private/long/tools/steam_monitor.py")
    ...
    expected = executable + " steam_monitor.py --setup" if method == "manual" else "steam_monitor --setup"
...
def test_help_does_not_display_runtime_paths():
    ...
    assert "steam_monitor.py --setup" in result.stdout

...
| `The Steam Web API did not answer in time`, `The Steam Web API could not be reached` or `The Steam Web API is temporarily unavailable` | A network problem between this machine and Steam or a Steam outage | [Connection Problems](#connection-problems) |
...
If the failure continues, check the internet connection, DNS and any firewall or proxy between this machine and Steam. A certificate error points at TLS interception on the network, see [TLS Verification](configuration.md#tls-verification). A server error that lasts is a Steam outage, so wait for it to end.
